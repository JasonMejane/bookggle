...
{
    if (g.game_over || cur_screen != SCREEN_GAME)
        return;
    ...
    if (!cell_from_coords(x, y, &row, &col))
        return;
...
{
    if (g.game_over || g.mode == MODE_MULTI)
        return;
